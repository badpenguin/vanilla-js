...
NodeList.prototype.first=function(){return this.item(0)};function $find(c,d){return c.querySelectorAll(d)}function $append(c){document.body.insertAdjacentHTML("beforeend",c)}function $disableScreenScrolling(){var c=window.innerWidth-document.documentElement.clientWidth;document.body.style.height="100vh";document.body.style.overflow="hidden";document.body.style.position="fixed";document.body.style.paddingRight=c+"px"}
function $restoreScreenScrolling(){document.body.style.height="initial";document.body.style.overflow="initial";document.body.style.position="initial";document.body.style.paddingRight="initial"}function $addClass(c,d){c.classList&&c.classList.add(d);c.className+=" "+d}function $removeClass(c,d){c.classList&&c.classList.remove(d);c.className=c.className.replace(RegExp("(^|\\b)"+d.split(" ").join("|")+"(\\b|$)","gi")," ")}
function $hasClass(c,d){return c.classList?c.classList.contains(d):RegExp("(^| )"+d+"( |$)","gi").test(c.className)}function $toggleClass(c,d){if(c.classList)c.classList.toggle(d);else{var a=c.className.split(" "),b=a.indexOf(d);0<=b?a.splice(b,1):a.push(d);c.className=a.join(" ")}}function $hide(c){c.style.display="none"}function $show(c,d){c.style.display=d?d:""}
function $detectSwipe(c,d,a){var b=0,g=0,h=0,k=0;a||(a=50);c.addEventListener("touchstart",function(a){b=a.changedTouches[0].screenX;g=a.changedTouches[0].screenY},!1);c.addEventListener("touchend",function(c){h=c.changedTouches[0].screenX;k=c.changedTouches[0].screenY;var e=h-b,f=k-g;if(Math.abs(e)>Math.abs(f)){if(Math.abs(e)>a)return 0>e?d("left",c):d("right",c)}else if(Math.abs(f)>a)return 0<f?d("down",c):d("up",c)},!1)}
