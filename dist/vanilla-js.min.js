...
if(c){var d=b.getAttribute("data-title")||b.getAttribute("title")||b.getAttribute("alt");if(!d){var e=$find(b,"img").first();e&&(d=e.getAttribute("data-title")||e.getAttribute("title")||e.getAttribute("alt"));if(!d)if(e=$find(b,"figcaption").first())d=e.innerText;else if((e=b.nextSibling)&&e.matches("figcaption"))d=e.innerText}a.images.push([c,d])}else console.warn("$lightbox: element has no valid image url",b)},checkImages:function(){$all(c).forEach(function(b){d?$find(b,d).forEach(function(b){a.scanElement(b)}):
...
a&&a(d,!0)):d.hasAttribute("data-visibile")&&(d.removeAttribute("data-visibile"),b.options.visibleClass&&$removeClass(d,b.options.visibleClass),b.options.invisibleClass&&$addClass(d,b.options.invisibleClass),a&&a(d,!1))})};b.nodeList=Array.prototype.slice.call($all(c));b.refreshList();0<b.nodeList.length?document.addEventListener("scroll",b.refreshList):console.warn("$visibility: no hidden elements");return{}};function $scrollDetector(c){function d(a){b!==a&&(b=a,$triggerEvent(window.document,"scroll-"+a))}var a=window.pageYOffset,b="";c=c?c:100;document.addEventListener("scroll",function(b){b=window.pageYOffset;if(Math.abs(b-a)>c){var f=b>a?"down":"up";a=b;return d(f)}if(0===b)return a=b,d("top");if(window.pageYOffset+window.innerHeight+c>(void 0!==document.height?document.height:document.body.offsetHeight))return a=b,d("bottom")})};
